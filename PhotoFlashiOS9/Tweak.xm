...
static void positionBtn(CAMTopBar *topBar) {
    if (btn) {
        if (IS_IPAD) {
            ...
            CGFloat originY = 22 - btnSize / 2;
            if (!rightBtn)
                btn.frame = CGRectMake(topBar.flashButton.frame.origin.x + topBar.flashButton.frame.size.width + 5, originY, btnSize, btnSize);
            else
...
static void createListButton(CAMTopBar *topBar) {
    btn = (CUShutterButton *)[%c(CUShutterButton) tinyShutterButton];
...
- (BOOL)pt_shouldHideYellowDot {
    ...
    return NO;
    #else
    return [self shouldHideFlashButtonForMode:__currentMode device:__currentDevice] || !flashSupportedForDevice(__currentDevice);
    #endif
}
...
- (id)initWithCaptureController: (CUCaptureController *)arg1 captureConfiguration: (id)arg2 conflictingControlConfiguration: (id)arg3 locationController: (id)arg4 motionController: (id)arg5 timelapseController: (id)arg6 keepAliveController: (id)arg7 remoteShutterController: (id)arg8 powerController: (id)arg9 {
    self = %orig;
    vf = self;
    return self;
}
...
- (void)stillImageRequestDidCompleteStillImageCapture:(id)arg1 withResponse:(id)arg2 error:(id)arg3 {
    %orig;
    if (torchMode && self._flashMode == 1 && isStillImageCamera(self) && ![self._captureController isCapturingBurst]) {
...
- (BOOL)pt_topBarShouldHideYellowDot: (id)arg1 {
    ...
    return NO;
    #else
    return [self pt_shouldHideYellowDotForMode:self._currentMode];
    #endif
}
...
- (BOOL)pt_shouldHideYellowDotForMode: (NSInteger)mode {
    ...
    return NO;
    #else
    return [self _shouldHideFlashButtonForMode:mode device:self._currentDevice] || !flashSupportedForDevice(self._currentDevice);
    #endif
}
...
- (void)_didChangeToMode:(NSInteger)mode device:(NSInteger)device shouldProcessIdenticalChanges:(BOOL)changes {
    %orig;
    if (!_isVideoMode(mode, device))
        setTorchValue(self);
}
